...
export function LLMScreeningPage() {
  const [activeFlow, setActiveFlow] = useState<'platform' | 'independent' | null>(null);
        ...
        <div className="mx-auto w-full max-w-7xl space-y-6 xl:max-w-[70vw]">
          ...
          <div className="flex items-center justify-end">
            <label
              ...
              className="flex items-center gap-2.5 cursor-pointer select-none rounded-lg border px-4 py-2.5 bg-surface-0 hover:bg-surface-2 transition-colors"
            >
              <Checkbox
                id="rationale-toggle"
          ...
          <div className="grid grid-cols-2 gap-5">
            <button
              ...
              className={cn(
                "relative flex flex-col items-center justify-center gap-3 rounded-xl border-2 px-6 py-8 bg-surface-0 cursor-pointer transition-all duration-200 outline-none",
                activeFlow === 'platform'
          ...
          {/* Flow content */}
          {activeFlow === 'platform' && <PlatformScreenFlow rationaleEnabled={rationaleEnabled} />}
          {activeFlow === 'independent' && <IndependentUseCaseFlow />}
        </div>
      </div>
    </div>
  );
